...
public class MyMazeGenerator extends AMazeGenerator{
    ...
    public Maze generate(int row, int col) {
        ...
        for(int r = 0; r < grid.max_rowSize; r++)
        {
            ...
            {
                //if((r != grid.getStartPosition().getRowIndex()) || (c != grid.getStartPosition().getColumnIndex())) {
                    grid.set_val(r, c, 1);
                //}
            }
        }
    ...
    private void Prim()
    {
        ...
        int startRow = this.grid.getStartPosition().getRowIndex();
        int startCol = this.grid.getStartPosition().getColumnIndex();
        if(this.grid.get_val(startRow, startCol) == 1)
        {
            if((startRow == 0) || (startRow == this.grid.max_rowSize))
            {
                for(int c = 0; c < grid.max_colSize; c++)
                {
                    if(this.grid.get_val(startRow, c) == 0)
                    {
                        //this.grid.set_val(startRow, c ,0);
                        this.grid.setStartPosition(startRow,c);
                        break;
                    }
                }
            }
            else if((startCol == 0)||(startCol == this.grid.max_colSize)){
                for(int r = 0; r < grid.max_rowSize; r++)
                {
                    if(this.grid.get_val(r, startCol) == 0)
                    {
                        //this.grid.set_val(r, startCol ,0);
                        this.grid.setStartPosition(r,startCol);
                        break;
                    }
                }
            }
